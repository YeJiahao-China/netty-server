 ...
 * 仅为方便本 echo 示例独立编译（因为主项目是 Spring Boot fat jar，
 * 无法直接作为 Maven 依赖被 import）。
 *
 * <p>运行时（jar 被加载到 netty-server 中）：
 *  - echo 协议 jar 的 URLClassLoader.parent 是主项目的 ClassLoader
 *  - JVM 按 parent-first 策略加载本接口，最终解析为主项目中的同一份接口
 *  - 因此 ServiceLoader 能正确识别 EchoProtocolProvider 实现
 *
 * <p>正式协议 jar 的推荐做法：
 *  - 在主项目 pom.xml 中给 spring-boot-maven-plugin 加 {@code <classifier>exec</classifier>}
 *  - 让主项目产出可被依赖的普通 jar
 *  - 外部协议 jar 直接 {@code <dependency>} 引用主项目（provided scope）
 *
 * @author yjh_c
 */
